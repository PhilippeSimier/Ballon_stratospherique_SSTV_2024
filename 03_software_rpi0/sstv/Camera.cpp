...
void Camera::envoyerPhoto(const unsigned long frequence, const std::string annotation){
    ...
    commande_convert << "convert -pointsize 20 -box white -draw \"text 10,20 '" << annotation;
...
void Camera::enregistrerPhoto(const std::string annotation){
    ...
    std::ostringstream commande_convert;
    commande_convert << "convert -pointsize 80 -box white -draw \"text 10,80 '"<< annotation << "  `date +\"%d/%m/%Y %T\"` ";
    commande_convert << "' \" ";
